...
def root_files() -> Dict[Path, str]:
    """
    What belongs at the top of sideeffects69.github.io, for a repository of that name.

    Crawlers look for robots.txt and llms.txt at the *root of a host*, and this
    site is a project page under /NovelForge/, so on its own it cannot be
    found by them however well it is written. GitHub serves the account's own
    root from a repository named `<account>.github.io`; these files are what to
    put there. Nothing here is published by this script.
    """
    lines = ["# sideeffects69.github.io: the only project here is NovelForge, at /NovelForge/.",
             "# Search engines and AI assistants are welcome.", "",
             "User-agent: *", "Allow: /", ""]
    for bot in CRAWLERS:
        lines += [f"User-agent: {bot}", "Allow: /", ""]
    lines += [f"Sitemap: {SITE}sitemap.xml",
              f"# Plain-text summary for language models: {SITE}llms.txt"]
    index = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>NovelForge: free novel writing software for Windows</title>
<meta name="description" content="Free, open-source novel writing software for Windows. This address forwards to the NovelForge website.">
<link rel="canonical" href="{SITE}">
<meta http-equiv="refresh" content="0; url={SITE}">
<link rel="alternate" type="text/plain" href="llms.txt" title="Plain-text summary for language models">
</head>
<body>
<p>NovelForge lives at <a href="{SITE}">{SITE}</a>.</p>
</body>
</html>
"""
    readme = f"""# Files for the root of sideeffects69.github.io
...
3. In that repository's Settings > Pages, publish from the `main` branch,
   folder `/ (root)`.
4. Check that {SITE.rsplit('NovelForge/', 1)[0]}robots.txt and
   {SITE.rsplit('NovelForge/', 1)[0]}llms.txt open in a browser.
...
"""
    return {
        ...
        HOST_ROOT / "llms.txt": llms(),
        HOST_ROOT / "index.html": index,
        HOST_ROOT / "README.md": readme,
    }
